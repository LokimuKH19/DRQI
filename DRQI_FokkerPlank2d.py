...
def print_config(model, seed, epochs, real_epoch, epsilon1, epsilon2, avg_time, timestamp, save_log=True, log_dir="logs"):
    ...
    standard = 5    # bugged in 1D and 2D and 3D 0.01 logs, should be 5 instead of d*pi^2
    # Luckily this problem does not affect the displayed results
    ...
    if save_log:
        ...
        filename = f"{model.algorithm}_FP_{model.d}D_SEED{seed}_{timestamp}.log"
...
def create_model(li, ff, d):
    mod = nn.Sequential()
    new_li = [2*d*ff] + li
    for i in range(len(new_li)):
        out_put = new_li[i+1] if i != len(new_li)-1 else 1
        mod.append(nn.Linear(new_li[i], out_put))
        mod.append(nn.Tanh())
    return mod
...
class MPINN(nn.Module):
    def __init__(self, d, algorithm):
        ...
        self.d = d
        self.fourier_feature = FOURIER_FEATURE[str(d)]
        self.net = create_model(NET_STRUCTURE[str(d)], FOURIER_FEATURE[str(d)], self.d)
    ...
    def u_th(self, x_list=None):
        ...
        # set your theoretical solution here
        sum_cos = torch.zeros_like(x_list[0])
        for xi in x_list:
            sum_cos += torch.cos(xi)
        return torch.exp(-torch.sin(0.1 * sum_cos))
    ...
    def potential(self, x):
        # x: list of tensors [x1, x2, ..., xd], each shape (N, 1)
        cos_sum = sum(torch.cos(xi) for xi in x)  # shape: (N, 1)
        return torch.sin(0.1 * cos_sum)  # shape: (N, 1)
        # nonlinear case (only 1d available by now)
        # c = 3.7846
        # return -torch.exp(2*torch.cos(x[0]))/c**2 + torch.sin(x[0])**2 - torch.cos(x[0])
    ...
    def net_forward(self, x):
        ...
        for i in range(self.d):
            xi = x[i]
            # Start j from 1, skip 0 to avoid sin(0) and cos(0)
            x1 += [torch.sin(j * xi) for j in range(1, self.fourier_feature + 1)]
            x1 += [torch.cos(j * xi) for j in range(1, self.fourier_feature + 1)]
    ...
    def update(self):
        ...
        for i in range(self.d):
            xi = x[i]  # shape (N, 1)
            # Gradient of u and v w.r.t xi
            u_xi = torch.autograd.grad(u, xi, grad_outputs=torch.ones_like(u), create_graph=True)[0]
            v_xi = torch.autograd.grad(v, xi, grad_outputs=torch.ones_like(v), create_graph=True)[0]
            u_xixi = torch.autograd.grad(u_xi, xi, grad_outputs=torch.ones_like(u_xi), create_graph=True)[0]
            v_xixi = torch.autograd.grad(v_xi, xi, grad_outputs=torch.ones_like(v_xi), create_graph=True)[0]
            u_xx_terms.append(u_xixi)
            v_xx_terms.append(v_xixi)
            uxvx_terms.append(u_xi * v_xi)
        # Stack and sum across dimension
        u_xx = torch.stack(u_xx_terms, dim=0).sum(dim=0)
        v_xx = torch.stack(v_xx_terms, dim=0).sum(dim=0)
        uxvx = torch.stack(uxvx_terms, dim=0).sum(dim=0)
        # Edit you operator here
        lu = - u_xx - uxvx - u*v_xx + 5*u
        # lu = - u_xx + u**3 + v*u    # nonlinear problem
        # Mode Selection
...
# Main Process
def main():
    global ALGORITHM, D, SEED, EPOCHS, EPSILON1, EPSILON2, LEARNING_RATE, OMEGA, GAMMA, NET_STRUCTURE, POINTS_PER_DIM, OPTIMIZER, FOURIER_FEATURE, config
    config = load_config()
    ...
    FOURIER_FEATURE = config["fourier_feature"]
    POINTS_PER_DIM = config["points_per_dimension"]
    OPTIMIZER = config["optimizer"]
    seed_everything(SEED)

    os.makedirs("models", exist_ok=True)
    model = MPINN(D, ALGORITHM)
    ...
    with open(f'models/{ALGORITHM}_FP_{D}D_OMEGA{model.unique_parameter["omega"]}_SEED{SEED}_Adam_{time_stamp}.pkl', 'wb') as fi:
